...
def test_setup_requires_argument(test_player):
    ...
    operation = testing_operation_module.TestingOperation(
        test_player, ui, command
    )
    operation.execute()
...
def test_setup_skilling_applies_and_prints_success(test_player):
    ui = CaptureUI()
    command = Command.parse('testing setup tools')
    ...
    assert ui.messages
    assert ui.messages[-1] == 'Applied testing setup `tools`.'
